...
function vmMonkeyPatch() {
    ...
    global.__magicpatchInternal.initialDir = process.cwd();
    global.__magicpatchInternal.ijavascriptMonkeyPatch = ijavascriptMonkeyPatch;
}
...
function printError(err) {
    if (!(err instanceof TypeError)) {
        // print the whole stack if it's not a type error
        console.error(err);
    } else if (err.message !== "") {
        // empty error message gives us a way to pop out of the stack without printing a message
        // used for commander, which prints its own error messages
